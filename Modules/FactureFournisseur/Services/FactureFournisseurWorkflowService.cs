...
public sealed class FactureFournisseurWorkflowService : IFactureFournisseurWorkflowService
{
    ...
    public async Task UpdatePaiementAsync(int factureFournisseurId, int paiementId, decimal montant, DateTime date, ModePaiement mode, string reference, bool estEncaisse, CancellationToken cancellationToken = default)
    {
        if (montant <= 0)
